...
A full-stack **Retrieval-Augmented Generation (RAG)** application built with a secure **Node.js Gateway**, a high-performance **FastAPI AI Service**, and a modern **Glassmorphism React Frontend**.

---

## 📸 Overview
**DocMind AI** allows users to upload documents (PDF, DOCX, TXT, MD), index them into a high-speed vector database, and query them using context-grounded AI answers. It is designed with a **Security-First** architecture, ensuring all AI interactions are authenticated and proxied.
...
*   **UI/UX**: Refined glassmorphism interface styled with CSS Variables and Lucide Icons.
*   **State Management**: `AuthContext` for secure session handling.
*   **Features**:
    *   **Quick Actions Bar**: Fast shortcuts to upload documents, jump to AI chat, or browse document library.
    *   **Document Library**: Displays document name, indexing status badges, chunk counts, and upload timestamps.
    *   **Suggested Prompts**: Clickable example questions ("What is this document about?", "Summarize the main points", etc.) to jumpstart conversations.
    *   **Source Citations**: Interactive source references grounded in retrieved document chunks.
    *   **User Feedback**: Clear loading indicators and friendly error alerts with dismiss controls.
